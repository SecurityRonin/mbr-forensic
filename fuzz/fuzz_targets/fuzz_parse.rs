...
fuzz_target!(|data: &[u8]| {
    // Must never panic.
    let result = mbr::parse_mbr_sector(data);

    if data.len() < 512 {
        assert!(
            matches!(result, Err(mbr::Error::TooShort(_))),
    ...
    match result {
        Ok(sector) => {
            ...
            let _ = mbr::boot_code::identify(&sector.boot_code);
        }
        Err(mbr::Error::BadSignature(_)) => {
            // Bytes 510-511 ≠ 0x55AA — expected.
        }
        Err(mbr::Error::TooShort(_)) => {
